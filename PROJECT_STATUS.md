...
当前更新链路已升级为非阻塞流程：点击下载后立即返回主界面，右上角持续显示下载进度；下载完成后自动退出、安装并重新启动，不再弹出第二次重启确认。1.0.19 已通过 GitHub Actions 自动构建并正式发布，EXE、blockmap 和 `latest.yml` 均已上传；用户已完成 1.0.18 → 1.0.19 真实跨版本验收，弹窗关闭、后台进度、自动安装与自动重启全部成功，本机安装版本亦确认是 1.0.19。
