...
当前更新链路已升级为非阻塞流程：点击下载后立即返回主界面，右上角持续显示下载进度；下载完成后自动退出、安装并重新启动，不再弹出第二次重启确认。代码版本为 1.0.19，安装包已成功构建且独立打包程序启动验证通过；当前电脑继续保留已安装的 1.0.18，以便发布 1.0.19 后进行真实跨版本自动更新验收。
