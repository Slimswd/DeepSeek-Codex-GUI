...
当前更新链路已升级为非阻塞流程：点击下载后立即返回主界面，右上角持续显示下载进度、实时下载速度与预计剩余时间；下载完成后自动退出、安装并重新启动，不再弹出第二次重启确认。1.0.20 已通过 GitHub Actions 自动构建并正式发布，EXE、blockmap 和 `latest.yml` 均已上传且公开下载验证通过；当前本机保持 1.0.19，待进行 1.0.19 → 1.0.20 的真实速度与剩余时间显示验收。
