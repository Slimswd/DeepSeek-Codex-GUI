...
最新开发版 1.0.23 已加入应用内更新日志摘要：发现新版本时会显示最多三条简明变更，并优先采用 GitHub Release 对应版本的 `CHANGELOG.md` 内容；GitHub Actions 已同步改为发布该版本说明，避免向用户展示原始提交记录。当前待本地打包与 GUI 验收，尚未正式发布。

当前更新链路已升级为非阻塞流程：点击下载后立即返回主界面，右上角持续显示下载进度、实时下载速度与预计剩余时间；下载完成后显示“立即安装 / 稍后”，由用户手动决定更新时机，普通关闭应用不会自动安装。若用户已经请求安装但仍有执行中、等待审批或排队任务，更新会暂缓到所有任务结束后再安装。1.0.22 已通过 GitHub Actions 自动构建并正式发布，EXE、blockmap 和 `latest.yml` 均已上传；待进行真实更新与任务保护验收。
