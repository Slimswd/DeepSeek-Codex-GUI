...
当前更新链路已升级为非阻塞流程：点击下载后立即返回主界面，右上角持续显示下载进度、实时下载速度与预计剩余时间；下载完成后显示“立即安装 / 稍后”，由用户手动决定更新时机，普通关闭应用不会自动安装。1.0.21 已通过 GitHub Actions 自动构建并正式发布，EXE、blockmap 和 `latest.yml` 均已上传；当前本机保持 1.0.19，待进行 1.0.19 → 1.0.21 的真实速度、剩余时间与手动安装验收。
