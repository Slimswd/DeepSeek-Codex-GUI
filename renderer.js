...
let latestGitStatus = null;
let currentGitProjectPath = null;

const gitStatusButton =
  document.createElement("button");

gitStatusButton.type = "button";
gitStatusButton.className =
  "git-status-trigger";
gitStatusButton.textContent = "Git --";
gitStatusButton.title =
  "查看当前项目 Git 状态和修改文件";
...
gitStatusPanel.className =
  "git-status-panel";

gitStatusPanel.innerHTML = `
  <div class="git-status-header">
    <div class="git-status-title">Git 状态与当前修改</div>
    <button type="button" class="git-status-close">×</button>
  </div>
  <pre class="git-status-summary"></pre>
  <div class="git-status-files"></div>
  <div class="git-status-actions">
    <button type="button" class="git-status-refresh">刷新</button>
  </div>
`;

const gitStatusSummary =
  gitStatusPanel.querySelector(
    ".git-status-summary"
  );
...
function renderGitStatus(snapshot) {
  latestGitStatus = snapshot || null;
  gitStatusFiles.innerHTML = "";

  if (!currentGitProjectPath) {
    gitStatusButton.textContent = "Git --";
    gitStatusSummary.textContent =
      "请先选择项目文件夹";
    return;
  }
  ...
  if (!snapshot.ok) {
    gitStatusButton.textContent =
      snapshot.message === "当前项目不是 Git 仓库"
        ? "Git 非仓库"
        : "Git 不可用";
    gitStatusSummary.textContent =
      `${snapshot.message || "读取 Git 状态失败"}\n项目：${snapshot.projectPath || "未选择"}`;

    const empty = document.createElement("div");
    empty.className = "git-status-empty";
    empty.textContent =
      "这里只读显示 Git 信息，不会执行提交或修改。";
    gitStatusFiles.appendChild(empty);
    return;
  }
  ...
  gitStatusButton.textContent =
    `Git ${summary.changed || 0}`;
  gitStatusSummary.textContent = [
    `分支：${snapshot.branch || "未命名"}`,
    `项目根目录：${snapshot.repositoryRoot || snapshot.projectPath}`,
    `变更：${summary.changed || 0} 个文件 · 暂存 ${summary.staged || 0} · 修改 ${summary.modified || 0} · 未跟踪 ${summary.untracked || 0}`,
    `冲突：${summary.conflicts || 0} · ${snapshot.upstream ? `领先 ${snapshot.ahead || 0} · 落后 ${snapshot.behind || 0}` : "未配置上游分支"}`
  ].join("\n");

  if (!files.length) {
    const empty = document.createElement("div");
    empty.className = "git-status-empty";
    empty.textContent = "工作区干净，没有未提交修改。";
    gitStatusFiles.appendChild(empty);
    return;
  }

  files.slice(0, 300).forEach(item => {
    const row = document.createElement("div");
    row.className = `git-file-row git-file-${item.kind || "modified"}`;
    ...
    filePath.textContent = item.path || "未知文件";
    filePath.title = item.path || "未知文件";

    row.appendChild(code);
    row.appendChild(filePath);
    gitStatusFiles.appendChild(row);
  });

  if (files.length > 300) {
    const more = document.createElement("div");
    more.className = "git-status-more";
    more.textContent = `还有 ${files.length - 300} 个文件未展开`;
    gitStatusFiles.appendChild(more);
  }
}
...
async function refreshGitStatus() {
  try {
    renderGitStatus(
      await window.deepseekCodex.getGitStatus()
    );
  } catch (error) {
    renderGitStatus({
      ok: false,
      projectPath: currentGitProjectPath,
      message: "读取 Git 状态失败：" + error.message
    });
  }
}

gitStatusButton.addEventListener("click", event => {
  event.stopPropagation();
  gitStatusPanel.style.display = "block";
  refreshGitStatus();
});

gitStatusPanel
  .querySelector(".git-status-close")
  .addEventListener("click", () => {
    gitStatusPanel.style.display = "none";
  });

gitStatusPanel
  .querySelector(".git-status-refresh")
  .addEventListener("click", refreshGitStatus);

document.body.appendChild(gitStatusPanel);

document.addEventListener("click", event => {
  if (
    !gitStatusPanel.contains(event.target) &&
    !gitStatusButton.contains(event.target)
  ) {
    gitStatusPanel.style.display = "none";
  }
});
...
document.head.appendChild(gitStatusStyle);
...
function renderState(state) {
  ...
  if (
    currentGitProjectPath !== (state.projectPath || null)
  ) {
    currentGitProjectPath =
